...
async function main() {
  console.log("📥 Memuat turun data Sunan an-Nasai (Arab, Inggeris & Indonesia/Melayu)...");

  try {
    const [arabicData, englishData, malayData] = await Promise.all([
      fetchJSON("https://cdn.jsdelivr.net/gh/fawazahmed0/hadith-api@1/editions/ara-nasai.json"),
      fetchJSON("https://cdn.jsdelivr.net/gh/fawazahmed0/hadith-api@1/editions/eng-nasai.json"),
      fetchJSON("https://cdn.jsdelivr.net/gh/fawazahmed0/hadith-api@1/editions/ind-nasai.json")
    ]);
    ...
    console.log(`✅ Berjaya memuat turun ${hadithArabic.length} hadis Nasai.`);
    ...
    console.log(`✅ Terdapat ${existingHadiths.size} hadis Nasai unik yang sudah siap diproses.`);
    ...
    for (let i = 0; i < limitHadith; i += batchSize) {
      ...
      for (let j = 0; j < batchArabic.length; j++) {
        if (!existingHadiths.has(`nasai:${batchArabic[j].hadithnumber}`)) {
      ...
      const requests = hadithsToProcess.map(h => ({
        content: { role: "user", parts: [{ text: `Sunan an-Nasai ${h.arabic.hadithnumber}: ${h.malay.text}` }] }
      }));
    ...
    console.log("🎉 Proses Ingestion Sunan an-Nasai Selesai!");
  } catch (error) {
