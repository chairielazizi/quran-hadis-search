...
async function main() {
  console.log("📥 Memuat turun data Sunan Abu Dawud (Arab, Inggeris & Indonesia/Melayu)...");

  try {
    const [arabicData, englishData, malayData] = await Promise.all([
      fetchJSON("https://cdn.jsdelivr.net/gh/fawazahmed0/hadith-api@1/editions/ara-abudawud.json"),
      fetchJSON("https://cdn.jsdelivr.net/gh/fawazahmed0/hadith-api@1/editions/eng-abudawud.json"),
      fetchJSON("https://cdn.jsdelivr.net/gh/fawazahmed0/hadith-api@1/editions/ind-abudawud.json")
    ]);
    ...
    console.log(`✅ Berjaya memuat turun ${hadithArabic.length} hadis Abu Dawud.`);
    ...
    console.log(`✅ Terdapat ${existingHadiths.size} hadis Abu Dawud unik yang sudah siap diproses.`);
    ...
    for (let i = 0; i < limitHadith; i += batchSize) {
      ...
      for (let j = 0; j < batchArabic.length; j++) {
        if (!existingHadiths.has(`abudawud:${batchArabic[j].hadithnumber}`)) {
      ...
      const requests = hadithsToProcess.map(h => ({
        content: { role: "user", parts: [{ text: `Sunan Abu Dawud ${h.arabic.hadithnumber}: ${h.malay.text}` }] }
      }));
    ...
    console.log("🎉 Proses Ingestion Sunan Abu Dawud Selesai!");
  } catch (error) {
