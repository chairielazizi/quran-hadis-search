...
async function main() {
  console.log("📥 Memuat turun data Hadis Muslim (Arab, Inggeris & Indonesia/Melayu)...");

  try {
    const [arabicData, englishData, malayData] = await Promise.all([
      fetchJSON("https://cdn.jsdelivr.net/gh/fawazahmed0/hadith-api@1/editions/ara-muslim.json"),
      fetchJSON("https://cdn.jsdelivr.net/gh/fawazahmed0/hadith-api@1/editions/eng-muslim.json"),
      fetchJSON("https://cdn.jsdelivr.net/gh/fawazahmed0/hadith-api@1/editions/ind-muslim.json")
    ]);
    ...
    console.log(`✅ Berjaya memuat turun ${hadithArabic.length} hadis Muslim.`);
    ...
    console.log(`✅ Terdapat ${existingHadiths.size} hadis Muslim unik yang sudah siap diproses.`);
    ...
    for (let i = 0; i < limitHadith; i += batchSize) {
      ...
      for (let j = 0; j < batchArabic.length; j++) {
        if (!existingHadiths.has(`muslim:${batchArabic[j].hadithnumber}`)) {
      ...
      const requests = hadithsToProcess.map(h => ({
        content: { role: "user", parts: [{ text: `Hadis Muslim ${h.arabic.hadithnumber}: ${h.malay.text}` }] }
      }));
    ...
    console.log("🎉 Proses Ingestion Hadis Muslim Selesai!");
  } catch (error) {
