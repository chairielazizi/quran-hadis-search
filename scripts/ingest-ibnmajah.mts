...
async function main() {
  console.log("📥 Memuat turun data Sunan Ibn Majah (Arab, Inggeris & Indonesia/Melayu)...");

  try {
    const [arabicData, englishData, malayData] = await Promise.all([
      fetchJSON("https://cdn.jsdelivr.net/gh/fawazahmed0/hadith-api@1/editions/ara-ibnmajah.json"),
      fetchJSON("https://cdn.jsdelivr.net/gh/fawazahmed0/hadith-api@1/editions/eng-ibnmajah.json"),
      fetchJSON("https://cdn.jsdelivr.net/gh/fawazahmed0/hadith-api@1/editions/ind-ibnmajah.json")
    ]);
    ...
    console.log(`✅ Berjaya memuat turun ${hadithArabic.length} hadis Ibn Majah.`);
    ...
    console.log(`✅ Terdapat ${existingHadiths.size} hadis Ibn Majah unik yang sudah siap diproses.`);
    ...
    for (let i = 0; i < limitHadith; i += batchSize) {
      ...
      for (let j = 0; j < batchArabic.length; j++) {
        if (!existingHadiths.has(`ibnmajah:${batchArabic[j].hadithnumber}`)) {
      ...
      const requests = hadithsToProcess.map(h => ({
        content: { role: "user", parts: [{ text: `Sunan Ibn Majah ${h.arabic.hadithnumber}: ${h.malay.text}` }] }
      }));
    ...
    console.log("🎉 Proses Ingestion Sunan Ibn Majah Selesai!");
  } catch (error) {
