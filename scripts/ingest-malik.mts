...
async function main() {
  console.log("📥 Memuat turun data Muwatta Malik (Arab, Inggeris & Indonesia/Melayu)...");

  try {
    const [arabicData, englishData, malayData] = await Promise.all([
      fetchJSON("https://cdn.jsdelivr.net/gh/fawazahmed0/hadith-api@1/editions/ara-malik.json"),
      fetchJSON("https://cdn.jsdelivr.net/gh/fawazahmed0/hadith-api@1/editions/eng-malik.json"),
      fetchJSON("https://cdn.jsdelivr.net/gh/fawazahmed0/hadith-api@1/editions/ind-malik.json")
    ]);
    ...
    console.log(`✅ Berjaya memuat turun ${hadithArabic.length} hadis Malik.`);
    ...
    console.log(`✅ Terdapat ${existingHadiths.size} hadis Malik unik yang sudah siap diproses.`);
    ...
    for (let i = 0; i < limitHadith; i += batchSize) {
      ...
      for (let j = 0; j < batchArabic.length; j++) {
        if (!existingHadiths.has(`malik:${batchArabic[j].hadithnumber}`)) {
      ...
      const requests = hadithsToProcess.map(h => ({
        content: { role: "user", parts: [{ text: `Muwatta Malik ${h.arabic.hadithnumber}: ${h.malay.text}` }] }
      }));
    ...
    console.log("🎉 Proses Ingestion Muwatta Malik Selesai!");
  } catch (error) {
